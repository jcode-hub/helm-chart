...
async function status(state) {
  core.info('foooooooooooooooooooooooo');
  try {
...
async function addTokenToKubeConfig(kubeconfigPath, kubeToken) {
  ...
  if (!(await fileExists(kubeconfigPath))) {
    throw new Error(
      'Cannot find a kubeconfig file, which is required when using helm2 with the kube-token option'
    );
  }
  ...
  if (!kubeconfigContent.contexts) {
    throw new Error(
      'Cannot find contexts in the kubeconfig file, which are required when using helm2 with the kube-token option'
    );
  }
  ...
  try {
    ...
    if (process.env.KUBECONFIG_FILE) {
      ...
      await writeFile(
        ...
        parseBase64(process.env.KUBECONFIG_BASE64)
      );
    }
    ...
    if (kubeToken) {
      if (helm === 'helm3') {
        ...
        process.env.KUBECONFIG = await addTokenToKubeConfig(
          process.env.KUBECONFIG,
          kubeToken
        );
      }
    ...
    if (repository) {
      ...
      const updateRepoArgs = ['repo', 'update'];
